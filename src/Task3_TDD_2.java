...
dots, spaces etc */
	...
	public void valid_list_seperator_test_1() {
		parser.add("output1", "o1", Parser.STRING);
		parser.parse("-o1 '1,2 3'"); // change here because "-o1=1,2 3" will only return 1,2 
	...
	public void valid_list_seperator_test_3() {
		parser.add("output1", "o1", Parser.STRING);
		parser.parse("-o1=1<2>3({)"); // change test from {}1<2>3({)
		List<Integer> list1 = Arrays.asList(1,2,3);
		List<Integer> result = parser.getIntegerList("o1");
		assertEquals(list1, result);
	}
//	
	...
	public void valid_list_seperator_test_4() {
		parser.add("output1", "o1", Parser.STRING);
		parser.parse("-o1=1,2,3mdkmd"); // change test from having special characters at the front to int at the front
		List<Integer> list1 = Arrays.asList(1,2,3);
		List<Integer> result = parser.getIntegerList("o1");
		assertEquals(list1, result);
	}
	...
	public void reapeated_bound() {
		parser.add("output1", "o1", Parser.STRING);
		parser.parse("-o1 '1-2 2-3'"); // "-o1= 1-2 2-3" will parse in 1-2 instead of 1-2 2-3
...
//	
	...
	public void reapeated_bound_with_negative_numbers() {
		parser.add("output1", "o1", Parser.STRING);
		parser.parse("-o1 '-1-2 2-3'"); // change this also because the same reason in line 184
	...
	public void long_range_test() {
		parser.add("output1", "o1", Parser.STRING);
		parser.parse("-o1='-1-2 3-5'"); // fix test cause this will only parse in 1-2 
...
//	
	...
	public void double_quote_sperator_test() {
		parser.add("output1", "o1", Parser.STRING);
		parser.parse("-o1=1\"2\"3\""); // fix because same problem with having special character in the front
		List<Integer> list1 = Arrays.asList(1,2,3);
		List<Integer> result = parser.getIntegerList("o1");
		assertEquals(list1, result);
		
		parser.add("output1", "o1", Parser.STRING);
		parser.parse("-o1=1'2'3'"); // getString for '1'2'3 will only return 1
		result = parser.getIntegerList("o1");
		assertEquals(list1, result);
	}
//		
